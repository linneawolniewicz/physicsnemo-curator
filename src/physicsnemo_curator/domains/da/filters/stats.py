...
class _MomentAccumulator:
    ...
    def _update_sample(self, x: np.ndarray) -> None:
        ...
        assert self._mean is not None  # noqa: S101
        ...
        n_old = self._n[finite]
        n_new = n_old + 1
        mean_old = self._mean[finite]
        x_f = x[finite]
        ...
        self._m3[finite] += term1 * delta_n * (n_new - 2) - 3.0 * delta_n * self._m2[finite]
        # Update M2
        self._m2[finite] += term1
        # Update mean
        self._mean[finite] += delta_n
        self._n[finite] = n_new

        # Min/max over finite observations only
        first = n_old == 0
        min_f = self._min[finite]
        max_f = self._max[finite]
        min_f = np.where(first, x_f, np.minimum(min_f, x_f))
        max_f = np.where(first, x_f, np.maximum(max_f, x_f))
        self._min[finite] = min_f
        self._max[finite] = max_f
